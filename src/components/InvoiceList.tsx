...
import { Printer, Trash2, Edit, Search, CheckCircle, Zap, Save, MessageCircle, XCircle, Send, PlayCircle } from 'lucide-react'
...
export default function InvoiceList({ invoices, categories, isHoldView = false }: { invoices: any[], categories: any[], isHoldView?: boolean }) {
  ...
  const filteredInvoices = invoices.filter(inv => {
    ...
    if (search) {
      ...
      match = match && (inv.id.toLowerCase().includes(q) || inv.customer.name.toLowerCase().includes(q) || (inv.customer.phone && inv.customer.phone.includes(q)))
    }
  ...
  const handleBulkWhatsApp = () => {
    ...
    if (validInvoices.length === 0) {
        alert("None of the selected customers have phone numbers saved.");
        return;
    }
    if (validInvoices.length < selected.length) {
        alert(`${selected.length - validInvoices.length} invoices were skipped because they lack phone numbers.`);
    }

    setWaQueue(validInvoices);
    setWaIndex(0);
    setSelectedIds([]); 
  }
  ...
  const handleSaveRecovery = async () => {
    const updates = Object.keys(draftPayments).map(id => ({ id, paidAmount: draftPayments[id] }))
    if (updates.length > 0) { await bulkUpdatePayments(updates); alert('All payments updated successfully!') }
    setRecoveryMode(false); setDraftPayments({})
  }
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full xl:w-auto bg-slate-50 p-2 rounded-xl border border-slate-200">
          <div className="relative flex-1 md:flex-none">
              ...
              <input type="text" placeholder="Search name/ID..." value={search} onChange={e => setSearch(e.target.value)} className="w-full md:w-48 pl-8 p-2 text-xs font-bold text-slate-900 outline-none bg-white rounded-lg border border-slate-200 focus:border-blue-500" />
          </div>
      ...
      <div className="flex justify-between items-center mb-4 min-h-[48px]">
          <div className="flex gap-3">
              {!selectedIds.length && waQueue.length === 0 && !isHoldView && (
                <button onClick={() => { setRecoveryMode(!recoveryMode); setDraftPayments({}) }} className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-black uppercase tracking-widest transition ${recoveryMode ? 'bg-slate-900 text-white shadow-lg' : 'bg-emerald-50 text-emerald-700 hover:bg-emerald-100 border border-emerald-200'}`}>
              ...
              {recoveryMode && !selectedIds.length && (
                  <button onClick={handleSaveRecovery} style={{ backgroundColor: '#059669', color: 'white' }} className="flex items-center gap-2 px-6 py-2 rounded-xl font-black uppercase tracking-widest text-xs hover:opacity-80 transition shadow-lg animate-fade-in">
                      <Save size={14} /> Save Entered Payments
                  </button>
              )}
          </div>
          ...
          {selectedIds.length > 0 && !recoveryMode && (
              <div className="bg-blue-600 text-white p-2 md:p-3 rounded-xl flex justify-between items-center w-full animate-fade-in shadow-lg">
                  ...
                  <div className="flex gap-3">
                      <button onClick={handleBulkWhatsApp} className="flex items-center gap-2 bg-green-500 text-white border border-green-400 px-3 md:px-4 py-1.5 md:py-2 rounded-lg text-[10px] md:text-xs font-black uppercase hover:bg-green-600 transition"><MessageCircle size={14}/> Bulk WhatsApp</button>
        ...
        <table className="w-full min-w-[1000px] text-left border-collapse">
          <thead className="bg-slate-100 text-slate-500 text-[10px] font-black uppercase tracking-widest border-b-2 border-slate-200">
            <tr>
              ...
              <th className={`p-4 text-right ${recoveryMode ? 'text-emerald-700 bg-emerald-100 border-b-4 border-emerald-400' : ''}`}>{recoveryMode ? 'ENTER PAID AMOUNT' : 'Paid'}</th>
          ...
          <tbody className="text-sm font-bold text-slate-700 divide-y divide-slate-100">
            {filteredInvoices.map((inv, index) => (
              <tr key={inv.id} className={`transition ${inv.isReturn ? 'bg-red-50 hover:bg-red-100' : 'hover:bg-slate-50'}`}>
                ...
                <td className="p-4 text-right text-orange-500 font-black">{inv.discountAmount > 0 ? `PKR ${inv.discountAmount.toLocaleString()}` : '---'}</td>
                    ...
                    {recoveryMode && !inv.isReturn ? (
                        <input 
                            ...
                            onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                    e.preventDefault();
                                    const nextField = document.getElementById(`payment-input-${index + 1}`);
                                    if (nextField) nextField.focus(); else handleSaveRecovery(); 
                                }
                            }}
                        />
                    ) : (
                ...
                <td className="p-4">
                  <div className="flex justify-end gap-2">
                    ...
                    <Link href={`/invoice/edit/${inv.id}`} className="p-2 bg-slate-100 text-slate-600 rounded hover:bg-slate-200 transition" title="Edit Invoice"><Edit size={16} /></Link>
