...
export default function InvoiceForm({ customers, products, initialData }: { customers: any[], products: any[], initialData?: any }) {
  ...
  useEffect(() => {
    if (initialData) {
        ...
        setRows(initialData.items.map((item: any, idx: number) => ({
            id: `edit-${idx}`, productId: item.productId, search: item.product.name, price: item.price.toString(), quantity: item.quantity, total: item.price * item.quantity
        })))
        getCustomerBalance(initialData.customer.id).then(bal => {
            ...
            if(initialData.isHold) invoiceImpact = 0; // Hold drafts don't impact past balances
  ...
  const handleSelectProduct = (index: number, prod: any) => {
    const newRows = [...rows]
    newRows[index].productId = prod.id; newRows[index].search = prod.name; newRows[index].price = prod.price ? prod.price.toString() : '' 
  ...
  const addRow = () => setRows([...rows, { id: Date.now().toString(), productId: '', search: '', price: '', quantity: 1, total: 0 }])
  ...
  // UPDATED SAVE FUNCTION FOR HOLD SUPPORT
  const handleSave = async (isHoldRequest: boolean = false) => {
    ...
    const data = { 
        ...
        isHold: isHoldRequest, // Passes Draft Status
  ...
  return (
    <div className="bg-white p-4 md:p-8 rounded-2xl shadow-xl border border-slate-200 relative">
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-white border-2 border-slate-200 shadow-2xl rounded-xl max-h-60 overflow-y-auto z-50 p-2">
                    {filteredCustomers.map((c, idx) => (
                        <div key={c.id} onClick={() => handleSelectCustomer(c)} onMouseEnter={() => setCustHoverIndex(idx)} className={`p-3 rounded-lg cursor-pointer text-slate-900 border-b border-slate-100 last:border-0 ${custHoverIndex === idx ? 'bg-blue-100' : 'hover:bg-blue-50'}`}>
                            <p className="font-black uppercase">{c.name}</p><p className="text-xs text-slate-500 font-bold">{c.phone} | ID: {c.id.slice(-4)}</p>
                        </div>
                    ))}
                </div>
            )}
        ...
        {rows.map((row, i) => (
          <div key={row.id} className="flex flex-col md:grid md:grid-cols-12 gap-4 items-start md:items-center bg-slate-50 p-4 md:p-3 rounded-xl border-2 border-slate-200 relative">
            <div className="w-full md:col-span-6 relative">
              ...
              <input id={`search-${i}`} type="text" placeholder="Search..." value={row.search} className="w-full p-3 md:p-2 bg-white border border-slate-300 rounded-lg font-black text-slate-900 outline-none focus:border-blue-500" onChange={(e) => { updateRowDetails(i, 'search', e.target.value); updateRowDetails(i, 'productId', ''); setActiveRowDrop(i); setProdHoverIndex(0) }} onFocus={(e) => { setActiveRowDrop(i); e.target.select() }} onKeyDown={(e) => handleProdKeyDown(e, i)} />
              {activeRowDrop === i && (
                  <div className="absolute top-full left-0 w-full mt-1 bg-white border-2 border-slate-300 shadow-2xl rounded-lg max-h-48 overflow-y-auto z-50 p-1">
                      {filteredProducts(i).map((p, idx) => (
                          <div key={p.id} onClick={() => handleSelectProduct(i, p)} onMouseEnter={() => setProdHoverIndex(idx)} className={`p-3 md:p-2 cursor-pointer rounded text-slate-900 ${prodHoverIndex === idx ? 'bg-blue-100' : 'hover:bg-slate-100'}`}><p className="font-black text-sm">{p.name}</p></div>
                      ))}
                  </div>
              )}
            </div>
      ...
      <button onClick={addRow} className="mt-4 w-full md:w-auto px-4 py-3 md:py-2 bg-slate-900 text-white rounded-lg font-black text-xs uppercase hover:bg-black transition">+ Add Row</button>
      ...
      {/* NEW BUTTON LAYOUT WITH HARD-FIXED COLORS */}
<div className="flex flex-col md:flex-row gap-4 mt-10">
    {!initialData?.isReturn && (
        <button 
            onClick={() => handleSave(true)} 
            type="button" 
            style={{ backgroundColor: '#f97316', color: 'white' }} // Hard-coded orange
            className="flex-1 py-5 rounded-xl font-black text-sm md:text-lg uppercase tracking-widest shadow-xl hover:opacity-90 transition"
        >
          Save as Quotation (Hold)
        </button>
    )}
    <button 
        onClick={() => handleSave(false)} 
        type="button" 
        style={{ backgroundColor: initialData?.isReturn ? '#dc2626' : '#2563eb', color: 'white' }} // Hard-coded Blue/Red
        className="flex-[2] py-5 rounded-xl font-black text-sm md:text-lg uppercase tracking-widest shadow-xl hover:opacity-90 transition"
    >
      {initialData ? (initialData.isReturn ? 'Update Return Record' : 'Update & Print Invoice') : 'Generate Invoice'}
    </button>
</div>
    </div>
  )
