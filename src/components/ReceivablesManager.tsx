...
export default function ReceivablesManager({ vouchers = [] }: { vouchers?: any[] }) {
  ...
  // AMENDED: Search by Customer Name and Customer ID (removed Voucher ID search)
  const filteredVouchers = safeVouchers.filter(v => {
    let match = true;
    if (search) {
        const q = search.toLowerCase()
        // Search by Customer Name and Customer ID (case-insensitive, real-time)
        match = match && (
        ...
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-6 mb-8 bg-slate-50 p-4 rounded-xl border border-slate-200">
            <div className="flex flex-wrap items-center gap-3 w-full">
                <div className="relative flex-1 min-w-[200px]">
                    ...
                    <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search Customer Name or ID..." dir="ltr" className="urdu-text text-left w-full pl-10 pr-4 py-2.5 bg-white border border-slate-300 rounded-lg text-sm font-bold text-slate-900 outline-none focus:border-blue-500 transition" />
                </div>
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200">
            <table className="w-full text-left border-collapse whitespace-nowrap">
                <thead className="bg-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-500 border-b border-slate-200">
                    {/* AMENDED: Changed "Voucher #" column to "Customer ID" */}
                    <tr><th className="p-4 w-12 text-center">Sr.</th><th className="p-4">Date</th><th className="p-4">Customer ID</th><th className="p-4">Customer</th><th className="p-4">Category</th><th className="p-4 min-w-[250px]">Notes / Comments</th><th className="p-4 text-right">Received</th><th className="p-4 text-right w-28">Actions</th></tr>
                </thead>
                <tbody className="divide-y divide-slate-100 text-sm font-bold text-slate-700">
                    {filteredVouchers.length > 0 ? filteredVouchers.map((v: any, index: number) => (
                        <tr key={v.id} className={`transition group ${editingId === v.id ? 'bg-blue-50' : 'hover:bg-slate-50'}`}>
                            ...
                            {/* AMENDED: Display Customer ID instead of Voucher ID */}
                            <td className="p-4 font-mono text-xs text-slate-900 font-black uppercase tracking-wide">{v.customer?.id}</td>
