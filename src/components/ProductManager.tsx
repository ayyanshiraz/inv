  ...
  return (
    <div className="max-w-5xl mx-auto bg-white p-6 md:p-10 rounded-3xl shadow-lg border border-slate-200">
      ...
      <form action={saveProduct} className="flex flex-col gap-5 mb-12" onSubmit={() => setTimeout(() => setEditingId(null), 100)}>
        ...
        {/* RESTORED: Cost AND Selling Price side-by-side */}
          ...
          <tbody className="text-sm font-bold text-slate-700 divide-y divide-slate-50">
            {products.map(p => (
              <tr key={p.id} className="hover:bg-slate-50 transition">
                ...
                <td className="py-5 px-2 text-emerald-600 font-black">PKR {p.price || 0}</td>
                <td className="py-5 px-2 flex justify-end gap-3">
                  <button onClick={() => setEditingId(p.id)} className="p-2 bg-blue-50 text-blue-600 rounded hover:bg-blue-100 transition" title="Edit"><Edit size={18} /></button>
                  <form action={deleteProduct.bind(null, p.id)}>
                      <button className="p-2 bg-red-50 text-red-600 rounded hover:bg-red-100 transition" title="Delete"><Trash2 size={18} /></button>
                  </form>
                </td>
              </tr>
            ))}
            {products.length === 0 && <tr><td colSpan={6} className="py-12 text-center text-slate-400 font-bold">No products found.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
