...
export default function VoucherForm({ customers }: { customers: any[] }) {
  ...
  const filteredCustomers = (index: number) => {
      ...
      return customers.filter(c => c.name.toLowerCase().includes(query) || (c.phone && c.phone.includes(query)) || (c.id && c.id.toLowerCase().includes(query)))
  }
  ...
  return (
    <div className="bg-white p-4 md:p-8 rounded-2xl shadow-xl border border-slate-200">
        ...
        <div className="flex justify-between items-center mt-4">
            <button type="button" onClick={addRow} className="px-4 py-2 bg-slate-100 text-slate-700 border border-slate-300 rounded-lg font-black text-[10px] uppercase hover:bg-slate-200 transition flex items-center gap-2"><Plus size={14} /> Add Row</button>
            <button type="button" onClick={handleSubmit} disabled={isSubmitting} className="px-8 py-3 bg-slate-900 text-white flex items-center gap-2 rounded-lg font-black uppercase tracking-widest text-xs hover:bg-black transition shadow-lg disabled:opacity-50"><Save size={16} /> {isSubmitting ? 'Saving...' : 'Save Vouchers'}</button>
        </div>
    </div>
  )
}