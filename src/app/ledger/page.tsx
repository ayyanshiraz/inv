...
export default async function LedgerPage({ searchParams }: { searchParams: Promise<{ from?: string, to?: string, view?: string, search?: string, category?: string }> }) {
  ...
  // --- REAL-TIME FILTERS (Strictly Typed) ---
  ...
  // NEW: Filter specifically for the Category Ledger View
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4 no-print">
          ...
          <form className="flex flex-wrap items-center gap-3 bg-white p-3 rounded-xl shadow-sm border border-slate-200 w-full md:w-auto">
              ...
              {/* CATEGORY FILTER - Now active for 'categories' view as well */}
              {(view === 'customers' || view === 'customer_list' || view === 'categories') && (
        ...
        <div className="flex justify-between items-end border-b-2 border-slate-200 pb-3 mb-6 no-print">
            <div className="flex gap-6 overflow-x-auto pb-2 custom-scrollbar">
                ...
                <Link href={`/ledger?from=${params.from || ''}&to=${params.to || ''}&view=categories`} className={`text-sm font-black uppercase tracking-widest pb-3 -mb-[14px] border-b-4 transition whitespace-nowrap ${view === 'categories' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-700'}`}>Category Ledger</Link>
                ...
                <Link href={`/ledger?from=${params.from || ''}&to=${params.to || ''}&view=inventory_list`} className={`text-sm font-black uppercase tracking-widest pb-3 -mb-[14px] border-b-4 transition whitespace-nowrap ${view === 'inventory_list' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-700'}`}>Product List</Link>
                <Link href={`/ledger?from=${params.from || ''}&to=${params.to || ''}&view=customer_list`} className={`text-sm font-black uppercase tracking-widest pb-3 -mb-[14px] border-b-4 transition whitespace-nowrap ${view === 'customer_list' ? 'border-slate-900 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-700'}`}>Customer List</Link>
            </div>
            <PrintButton />
        </div>
        ...
        <div id="print-area" className="bg-white border border-slate-200 rounded-xl shadow-sm print:shadow-none print:border-none print:rounded-none">
          <div className="p-6 md:p-8">
              ...
              <div className="w-full overflow-x-auto">
                  ...
                  {/* VIEW 2: CATEGORIES (EXPANDED GROUP VIEW) */}
                  {view === 'categories' && (
                      <table className="w-full min-w-[800px] text-left border-collapse border border-slate-300">
                      <thead className="bg-slate-100 text-slate-800 text-[10px] font-black uppercase tracking-widest border-b-2 border-slate-300">
                          <tr>
                      ...
                      {displayCategories.map((cat: any) => (
                          <tbody key={cat.category} className="text-xs font-bold text-slate-700 border-b-4 border-slate-400">
                              {/* 1. Category Summary Row (Shrinked View) */}
                              ...
                              {/* 2. Expanded Customer Details under this Category */}
                  ...
                  {view === 'products' && (
                      ...
                      <tbody className="text-xs font-bold text-slate-700">
                          {displayProducts.map((p: any, index: number) => (<tr key={index} className="hover:bg-slate-50 transition"><td className="p-3 border border-slate-300 text-center text-slate-400">{index + 1}</td><td className="p-3 border border-slate-300 uppercase whitespace-nowrap text-slate-900">{p.name}</td><td className="p-3 border border-slate-300 whitespace-nowrap">{p.category}</td><td className="p-3 border border-slate-300 text-center font-black text-slate-900">{p.qty}</td><td className="p-3 border border-slate-300 text-right text-blue-700">{p.revenue.toLocaleString()}</td><td className="p-3 border border-slate-300 text-right text-emerald-600 font-black">{(p.revenue - p.cost).toLocaleString()}</td></tr>))}
                  ...
                  {view === 'inventory_list' && (
                      ...
                      <tbody className="text-xs font-bold text-slate-700">
                          {displayInventory.map((p: any, index: number) => (<tr key={p.id} className="hover:bg-slate-50 transition"><td className="p-3 border border-slate-300 text-center text-slate-400">{index + 1}</td><td className="p-3 border border-slate-300 uppercase text-slate-900">{p.name}</td><td className="p-3 border border-slate-300">{p.category}</td><td className="p-3 border border-slate-300 text-right text-red-600">PKR {p.cost.toLocaleString()}</td><td className="p-3 border border-slate-300 text-right text-emerald-600 font-black">PKR {p.price?.toLocaleString() || 0}</td></tr>))}
                      </tbody>
                      </table>
                  )}

                  {/* VIEW 5: CUSTOMER LIST */}
                  {view === 'customer_list' && (
                      <table className="w-full min-w-[700px] text-left border-collapse border border-slate-300">
                      <thead className="bg-slate-100 text-slate-800 text-[10px] font-black uppercase tracking-widest border-b-2 border-slate-300">
